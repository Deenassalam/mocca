    ...
    <section class="portfolio" id="portfolio">
      <div class="container">
        ...
        @foreach($data as $datas)
            <form action="{{ route('komunitas.update', $datas->id_kom) }}" method="POST">
                {{ csrf_field() }}
                {{ method_field('PUT') }}
                <div class="form-group">
                    <label for="id_relawan">ID Komunitas:</label>
                    <input type="" class="form-control" id="usr" name="id_kom" value="{{ $datas->id_kom }}"readonly="true">
                </div>
                <div class="form-group">
                    <label for="nama">Nama Komunitas:</label>
                    <input type="text" class="form-control" name="nama_kom" value="{{ $datas->nama_kom }}">
                </div>
                 <div class="form-group">
                    <label for="status">Nama Ketua Komunitas:</label>
                    <input type="text" class="form-control" id="status" name="nama_ket" value="{{ $datas->nama_ketua }}">
                </div>
                <div class="form-group">
                    <label for="email">Email Ketua Komunitas:</label>
                    <input type="email" class="form-control" id="email" name="email_ket" value="{{ $datas->email_ketua }}">
                </div>
               
                <div class="form-group">
                    <label for="nohp">No. Telepon Ketua Komunitas:</label>
                    <input type="text" class="form-control" id="nohp" name="hp_ket" value="{{ $datas->hp_ketua}}">
                </div>
